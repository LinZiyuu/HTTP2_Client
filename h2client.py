...
def tls_setup_exchange(dn, port):
    ...
    ip_and_port = l[0][4]
    print("l[0]:{}\nL[1]:{}\nL[2]:{}\nL[3]:{}\nL[4]:{}\n".format(l[0][0], l[0][1], l[0][2], l[0][3], l[0][4]))
    ...
    # connect the socket to the TCP endpoint.
    ssl_sock.connect(ip_and_port)
    # test alpn protocol
    print("alpn protocol:", ssl_sock.selected_alpn_protocol())
    assert ('h2' == ssl_sock.selected_alpn_protocol())
...
def send_sequence(sock, frames=None):
    if not frames:
        print( b'no frame to send.')
    else:
        # 创建帧序列并且发送
        sequence = h2.H2Seq()
    ...
    while True:
        try:
            ...
            new_frame = None
    # 返回响应
    print(b'response-code: ' + b','.join(status_codes) + b'\r\nerror: ' + b','.join(
        error_codes)  + b'\r\n\r\n' + response_data)


if __name__ == '__main__':
    ssl_stream_sock = tls_setup_exchange(dn = '172.17.168.200', port=443)
    initial_h2_exchange(sock=ssl_stream_sock)
    # frames_dict = {"<headers-frame-base-2:1>": {":method": "POST", ":scheme": "https", ":path": "/", ":authority": "47.96.175.128", "flags": []},
    ...
    for anomaly_name, frame_dict in frames_dict.items():
        for frame_name, frame_fileds in frame_dict.items():
            ...
            frame.show()
            frames.append(frame)
    send_sequence(sock=ssl_stream_sock, frames=frames)
    # for frame_name, frame_fileds in frames_dict.items():
    #     frame_type = extract_type(frame_name=frame_name)
    #     frame = build_frame(fileds_dict=frame_fileds, frame_type=frame_type)
    #     frame.show()
    #     frames.append(frame)
